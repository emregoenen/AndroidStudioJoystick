...
public class balance_joystick extends View implements Runnable{
    ...
    */
     ...
     * balance_joystick's button is moved
     */
    public interface OnMoveListener {

        /**
         * Called when a balance_joystick's button has been moved
    ...
    /**
     * Interface definition for a callback to be invoked when a balance_joystick
     * is touched and held by multiple pointers.
     */
    public interface OnMultipleLongPressListener {
        /**
         * Called when a balance_joystick has been touch and held enough time by multiple pointers.
         */
    ...
    */
    ...
    /**
     * Simple constructor to use when creating a balance_joystick from code.
     * Call another constructor passing null to Attribute.
     * @param context The Context the balance_joystick is running in, through which it can
     *        access the current theme, resources, etc.
     */
    public balance_joystick(Context context) {
        this(context, null);
    }


    public balance_joystick(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs);
    }


    /**
     * Constructor that is called when inflating a balance_joystick from XML. This is called
     * when a balance_joystick is being constructed from an XML file, supplying attributes
     * that were specified in the XML file.
     * @param context The Context the balance_joystick is running in, through which it can
     *        access the current theme, resources, etc.
     * @param attrs The attributes of the XML tag that is inflating the balance_joystick.
     */
    public balance_joystick(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray styledAttributes = context.getTheme().obtainStyledAttributes(
                attrs,
                R.styleable.balance_joystick,
                0, 0
        );
        ...
        try {
            buttonColor = styledAttributes.getColor(R.styleable.balance_joystick_JV_buttonColor, DEFAULT_COLOR_BUTTON);
            borderColor = styledAttributes.getColor(R.styleable.balance_joystick_JV_borderColor, DEFAULT_COLOR_BORDER);
            mBorderAlpha = styledAttributes.getInt(R.styleable.balance_joystick_JV_borderAlpha, DEFAULT_ALPHA_BORDER);
            backgroundColor = styledAttributes.getColor(R.styleable.balance_joystick_JV_backgroundColor, DEFAULT_BACKGROUND_COLOR);
            borderWidth = styledAttributes.getDimensionPixelSize(R.styleable.balance_joystick_JV_borderWidth, DEFAULT_WIDTH_BORDER);
            mFixedCenter = styledAttributes.getBoolean(R.styleable.balance_joystick_JV_fixedCenter, DEFAULT_FIXED_CENTER);
            mAutoReCenterButton = styledAttributes.getBoolean(R.styleable.balance_joystick_JV_autoReCenterButton, DEFAULT_AUTO_RECENTER_BUTTON);
            mButtonStickToBorder = styledAttributes.getBoolean(R.styleable.balance_joystick_JV_buttonStickToBorder, DEFAULT_BUTTON_STICK_TO_BORDER);
            buttonDrawable = styledAttributes.getDrawable(R.styleable.balance_joystick_JV_buttonImage);
            mEnabled = styledAttributes.getBoolean(R.styleable.balance_joystick_JV_enabled, true);
            mButtonSizeRatio = styledAttributes.getFraction(R.styleable.balance_joystick_JV_buttonSizeRatio, 1, 1, 0.25f);
            mBackgroundSizeRatio = styledAttributes.getFraction(R.styleable.balance_joystick_JV_backgroundSizeRatio, 1, 1, 0.75f);
            mButtonDirection = styledAttributes.getInteger(R.styleable.balance_joystick_JV_buttonDirection, BUTTON_DIRECTION_BOTH);
        } finally {
    ...
    /**
     * Set the button color for this balance_joystick.
    ...
    /**
     * Set the border color for this balance_joystick.
    ...
    /**
     * Set the border alpha for this balance_joystick.
    ...
    /**
     * Set the background color for this balance_joystick.
    ...
    /**
     * Set the border width for this balance_joystick.
    ...
    /**
     * Register a callback to be invoked when this balance_joystick's button is moved
    ...
    /**
     * Register a callback to be invoked when this balance_joystick is touch and held by multiple pointers
     * @param l The callback that will run
     */
